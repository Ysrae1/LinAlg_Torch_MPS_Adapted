...
class LQRSol_SO:
    ...
    def is_positive_definite(self, matrix):
        ...
        eigvals, _ = torch.linalg.eig(matrix.to('cpu'))
    ...
    def riccati_solver(self, time_grids, test = 'C'):
        
        if not isinstance(time_grids, torch.Tensor):
            raise TypeError("time_grid should be an batch_size*1-D torch.Tensor")
        else:
            if not (torch.all(torch.abs(time_grids[:,-1] - self.T) <= 1e-12) or torch.all(time_grids[:,0]>=0)):
                print()
        ...
        repl = torch.ones(time_grids_in.shape,device=self.device)
        ...
        for i in range(dt.shape[1]):
            ...
            if self.method == "euler":
                S_repl[:,i+1] = self.euler_step(S_repl[:,i], dt[:,i], test)
            elif self.method == "rk4":
                S_repl[:,i+1] = self.rk4_step(S_repl[:,i], dt[:,i], test)
            else:
                raise ValueError("Unsupported method")

        return torch.flip(S_repl, dims=[1])

    def euler_step(self, S_in, dt_in, test):
        
        # Correct version

        if test == 'C':

            dS_in = - (S_in @self.H + self.H.T @ S_in) + S_in @ self.M @ torch.inverse(self.D) @ self.M.T @ S_in - self.C
    ...
    def rk4_step(self, S_in, dt_in, test):

        def riccati_derivative(S_in,test):

            if test == 'C':

                return  - (S_in @self.H + self.H.T @ S_in) + S_in @ self.M @ torch.inverse(self.D) @ self.M.T @ S_in - self.C
        ...
        k1 = riccati_derivative(S_in, test)
        k2 = riccati_derivative(S_in + 0.5 *  k1 * dt_resized, test)
        k3 = riccati_derivative(S_in + 0.5 *  k2 * dt_resized, test)
        k4 = riccati_derivative(S_in + k3 * dt_resized, test)
    ...
    def value_function(self, t_batch, x_batch, sol_method = 'interpolation'):
        ...
        if sol_method == 'direct_calcul':
            ...
            time_grids = torch.stack([torch.linspace(float(t), self.T, self.N_step) for t in t_batch])

            S_tensor_tensor = self.riccati_solver(time_grids)
            ...
            trace_for_int = torch.einsum('bcii->bc', self.sigma @ self.sigma.T @ S_t_T.transpose(0,1)).unsqueeze(1).unsqueeze(2)
    ...
    def markov_control(self, t_batch, x_batch, sol_method = 'interpolation'):
        ...
        if sol_method == 'interpolation':
            ...
            time_grid = torch.stack([torch.linspace(0, self.T, N_step) for i in [0]])
        ...
        if sol_method == 'direct_calcul':
            ...
            time_grids = torch.stack([torch.linspace(float(t), self.T, self.N_step, dtype=torch.float32) for t in t_batch])
